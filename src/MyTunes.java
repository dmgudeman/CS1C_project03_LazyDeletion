...
public class MyTunes 
{
	 ...
	 */
	private LazySearchTree<SongEntry> purchasedTunes;
	
	/*
	 * Where all songs from MillionSongSubset are stored.
	 * 
	 * Note: Need to modify to use our Binary Search Tree called "LazySearchTree".
	 */
	...
	public static void main(String[] args) {
		...
		LazySearchTree<SongEntry> storeTitles = store.getTitles();
		System.out.println("Welcome! We have over " +  storeTitles.sizeHard() + " in FoothillTunes Jukebox!");
		...
		for (int i = 0; i < linesInFile.size() && selection != QUIT; /*no need to increment here */) 
		{
			...
			case LIST_SONG_TITLES:
				System.out.println("Number of titles in store = " + personalTunes.getPurchasedTunes().sizeHard());
			...
			case CREATE_RANDOM_PLAYLIST:
				...
				// From previous project 
				LazySearchTree<SongEntry> myPlayList = personalTunes.makePlayList(seconds);
				...
				// report algorithm time
				// QUESTION: How much did we improve by using a BinarySearchTree in our FoothillTunesStore?
				System.out.println("\nAlgorithm Elapsed Time: "
						+ TimeConverter.convertTimeToString(estimatedTime) + "\n");
				break;
			case ADD_SONG_TO_JUKEBOX:
				...
				// to simplify implementation we will assume the root node is what the user wants
				SongEntry songFound = personalTunes.containsTitle(title);
				if (songFound == null)
				{
					System.out.println("Warning! requested song " + title + "  not found.");
					continue;
				}
